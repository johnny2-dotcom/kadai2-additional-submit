...
def main():
    ...
    while True:
        ...
        xp_text = root.cssselect('tr:contains("初年度年収") > td.tableCondition__body')
        
        # 1ページ分繰り返し
        for name, copy, status, elm in zip(name_list, copy_list, status_list,xp_text):
            try:
                # try~exceptはエラーの可能性が高い箇所に配置
                exp_name_list.append(name.text)
                exp_copy_list.append(copy.text)
                exp_status_list.append(status.text)
                exp_first_year_fee_list.append(elm.text)
                # 初年度年収をtableから探す
